...
def histolog_multiples_r(Tableau1, Tableau2, Tableau3):
    ...
    ax.hist(Temps1, bins=logintervalles, density=1, alpha = 0.8, label=r'$r=%.3f$' %r1, color = couleurs[0])
    ax.hist(Temps2, bins=logintervalles, density=1, alpha = 0.8, label=r'$r=%.1f$' %r2, color = couleurs[1])
    ax.hist(Temps3, bins=logintervalles, density=1, alpha = 0.4, label=r'$r=%.1f$' %r3, color=couleurs[2])
    ax.set_xscale('log')
    ...
    ax.vlines(maxi1, ymin = 0, ymax = 0.175, color = couleurs[0], label = r'max ($r=%.3f$)' %r1, linewidth = 5, alpha=0.7)
    ax.vlines(maxi2, ymin = 0, ymax = 0.175, color = couleurs[1], label = r'max ($r=%.1f$)' %r2, linewidth = 5, alpha=0.7)
    ax.vlines(maxi3, ymin = 0, ymax = 0.175, color = couleurs[2], label = r'max ($r=%.1f$)' %r3, linewidth = 5, alpha=0.7)


    ax.legend(prop={'size':15},bbox_to_anchor=(0.5, 1.24), loc='upper center', ncol=4)
...
def histolog_multiples_f(Tableau1, Tableau2, Tableau3):
    ...
    plt.hist(Temps1, bins=logintervalles, density=1, alpha = 0.8, label=r'$f=%.d$' %f1, color = couleurs[0])
    plt.hist(Temps2, bins=logintervalles, density=1, alpha = 0.8, label=r'$f=%.1f$' %f2, color = couleurs[1])
    plt.hist(Temps3, bins=logintervalles, density=1, alpha = 0.4, label=r'$f=%.d$' %f3, color=couleurs[2])
    plt.xscale('log')
    ...
    plt.vlines(maxi1, ymin = 0, ymax = 0.175, color = couleurs[0], label = r'max ($f=%.3f$)' %f1, linewidth = 5, alpha=0.7)
    plt.vlines(maxi2, ymin = 0, ymax = 0.175, color = couleurs[1], label = r'max ($f=%.1f$)' %f2, linewidth = 5, alpha=0.7)
    plt.vlines(maxi3, ymin = 0, ymax = 0.175, color = couleurs[2], label = r'max ($f=%.1f$)' %f3, linewidth = 5, alpha=0.7)


    plt.legend(prop={'size':15},bbox_to_anchor=(0.5, 1.24), loc='upper center', ncol=4)
    plt.subplots_adjust(top=0.8)
    plt.show()
    plt.savefig("FigS4_Histo_contresel_f_s01.png")


#==================================
#To compute the values of rho, and specify them in the legend :

#For the partial dominance scenario
def rho_contresel(r, pin, h, s, f):
    ...
    rho = 0.25*(beta + np.sqrt(delta))

    return rho


#For the overdominance scenario
def rho_overdom(r, pin, s3, s4, f):
