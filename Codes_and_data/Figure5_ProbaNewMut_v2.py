...
def trace_probanewmut(distance, taux_mut_bp, Tableau):
    ...
    plt.plot(Echelle_R, les_probas[0:nb_R], color = Couleurs[0], linestyle = Styles[0], linewidth=2, label = r'$f=%.d$' %(F[0]))
    #Then all the others curves are plotted
    for i in range(1,nb_f):
        plt.plot(Echelle_R, les_probas[i*nb_R:(i+1)*nb_R], color = Couleurs[i], linestyle = Styles[0], linewidth=2, label = r'$f=%.1f, p_{in}=%.1f$' %(F[i*nb_R], Pin[0]))
        plt.plot(Echelle_R, les_probas[nb_f*nb_R+i*nb_R:nb_f*nb_R+(i+1)*nb_R], color = Couleurs[i], linestyle = Styles[1], linewidth=2, label = r'$f=%.1f, p_{in}=%.1f$' %(F[i*nb_R+nb_f*nb_R], Pin[nb_f*nb_R]))
    ...
    plt.xlabel(r'Recombination rate between the mating-type locus and the first mutation', fontsize=20)
...
taux_mut_bp = 10**(-8)
distance = 10**6
trace_probanewmut(distance, taux_mut_bp, Tglobal)